...
function futurio_theme_stylesheets() {
	wp_enqueue_style( 'futurio-bootstrap', get_template_directory_uri() . '/css/bootstrap.css', array(), '3.3.7' );
	// Theme stylesheet.
	wp_enqueue_style( 'futurio-stylesheet', get_stylesheet_uri(), array( 'futurio-bootstrap' ), FUTURIO_VERSION );
...
function futurio_theme_js() {
	wp_enqueue_script( 'futurio-bootstrap', get_template_directory_uri() . '/js/bootstrap.min.js', array( 'jquery' ), '3.3.7', true );
...
/**
 * Recommended plugins in customizer
 */
...
if ( class_exists( 'WooCommerce' ) ) {
	...
	if ( !function_exists( 'futurio_cart_link' ) ) {
		...
		function futurio_cart_link() {
			?>	
			<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_html_e( 'View your shopping cart', 'futurio' ); ?>">
				<i class="fa fa-shopping-bag"><span class="count"><?php echo is_object( WC()->cart ) ? wp_kses_data( WC()->cart->get_cart_contents_count() ) : ''; ?></span></i>
			</a>
			<?php
		}
		...
		if ( futurio_get_meta( 'content_layout' ) ) {
			if ( futurio_get_meta( 'content_layout' ) == 'fullwidth' ) {
				...
				<div class="container-fluid main-container" role="main">
					<div class="page-area">
				<?php 
			} elseif ( futurio_get_meta( 'content_layout' ) == 'fullwidth_builders' ) { ?>
...
if ( !function_exists( 'futurio_generate_header' ) ) :
	...
	function futurio_generate_header( $topnav = '', $header = '', $mainmenu = '', $thumbimg = '', $breadcrumbs = '', $openingdiv = '' ) {
		...
		if ( get_theme_mod( 'custom_header', '' ) != '' && in_array( 'elementor/elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
...
/**
	* Automatic update checker - before submit on WP.org, will be removed after approve
	*/
require ( trailingslashit( get_template_directory() ) . 'lib/plugin-update-checker/plugin-update-checker.php' );
$myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'https://futuriowp.com/wp-content/uploads/download/theme.json',
