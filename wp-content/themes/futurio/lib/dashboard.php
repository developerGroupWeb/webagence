...
if ( !class_exists( 'Futurio_Notify_Admin' ) ) :
	...
	class Futurio_Notify_Admin {
		...
		public function enqueue_scripts() {
			...
			if ( isset( $wp_customize ) || true === (bool) get_option( 'futurio_notify_dismissed' ) || futurio_is_extra_activated() ) {
				return;
			}

			wp_enqueue_style( 'futurio-admin', get_template_directory_uri() . '/css/admin/admin.css', '', '1' );

			wp_enqueue_script( 'futurio-admin', get_template_directory_uri() . '/js/admin/admin.js', array( 'jquery' ), '1', 'all' );
		...
		public function admin_notices() {
			...
			if ( true === (bool) get_option( 'futurio_notify_dismissed' ) || futurio_is_extra_activated() ) {
				return;
			}
			?>
